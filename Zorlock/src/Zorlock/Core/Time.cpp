...
namespace Zorlock
{
	...
	UINT32 Time::_getFixedUpdateStep(UINT64& step)
	{
		...
		if (nextFrameTime <= currentTime)
		{
			...
			auto numIterations = (UINT32)MathF::divideAndRoundUp(simulationAmount, (INT64)mFixedStep);
			...
			{
				stepus = MathF::divideAndRoundUp(simulationAmount, (INT64)mNumRemainingFixedUpdates);
				numIterations = (UINT32)MathF::divideAndRoundUp(simulationAmount, (INT64)stepus);
			}
