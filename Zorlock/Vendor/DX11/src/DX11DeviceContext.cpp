...
namespace ZorlockDX11
{
	...
	void DX11DeviceContext::setviewportsize(int width, int height)
	{
		D3D11_VIEWPORT vp = {};
		vp.Width = width;
		vp.Height = height;
	...
	bool DX11DeviceContext::setstencilstate()
	{
		...
		HRESULT hr = DX11GraphicsEngine::get()->GetD3D11Device()->CreateDepthStencilState(&depthstencildesc, &m_depth_stencilstate);
		if (FAILED(hr))
		{
	...
	bool DX11DeviceContext::setsamplerstate()
	{
		...
		HRESULT hr = DX11GraphicsEngine::get()->GetD3D11Device()->CreateSamplerState(&sampler_desc, &m_sampler_state);
		if (FAILED(hr))
		{
